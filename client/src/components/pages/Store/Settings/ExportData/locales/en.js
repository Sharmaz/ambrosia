const exportDataEn = {
  cardExportData: {
    title: "Export data",
    description: "Download a backup of your business data (products, orders, users, and configuration) to restore later or migrate to another device.",
    passwordNotice: "Your backup will be protected with your current wallet password — you'll need it to restore this data later.",
    exportButton: "Export data",
    exporting: "Preparing your backup...",
    phasePreparing: "Preparing your backup...",
    phaseWriting: "Writing your backup...",
    modalTitle: "Confirm access",
    passwordLabel: "Wallet password",
    cancelButton: "Cancel",
    confirmButton: "Confirm",
    success: "Backup downloaded successfully.",
    errorDescription: "Wrong password or error creating the backup.",
  },
};

export default exportDataEn;
